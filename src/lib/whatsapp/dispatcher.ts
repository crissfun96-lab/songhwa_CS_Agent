  ...
  try {
    switch (name) {
      ...
      case "create_reservation": {
        ...
        // Confirm using the STORED reservation (canonical resolved date) when available, not
        // the model's raw args, so the confirmation can never state a different date than booked.
